...
RUN apt-get -y install nginx supervisor git vim wget nmap dpkg-dev debhelper libpam-dev libcap2-dev libldap2-dev libmysqlclient-dev libpq-dev libssl-dev po-debconf postgresql && \
...
ADD ./config/pure-ftpd-postgresql.conf /etc/pure-ftpd/db/postgresql.conf
ADD ./config/postgres-pg-hba.conf /postgres-pg-hba.conf
